    ...
    def __init__(
        ...
        if len(parent_variable_names) == 0:
            ...
            if marginal_pdf is None or parameters is None:
                raise ValueError(
                    "Root nodes must have a marginal probability distribution and parameters."
                )
            ...
            if parameters is None:
                self.parameters = {"locs": {"intercept": 0, "slope": 1}, "scale": 1}
            else:
                self.parameters = parameters
    ...
    def __init__(self, node: Node) -> None:
        ...
        if node.type == NodeType.ROOT:
            ...
            def pdf(values: dict[str, float]) -> float:
                variable_value = values[node.variable_name]
                parent_values = {
                    parent: values[parent] for parent in node.parent_variable_names
                }
    ...
    def __mul__(self, other):
        ...
        def new_pdf(values: dict[str, float]) -> float:
            factor1_variable_values = {
                key: value for key, value in values.items() if key in self.scope
            }
            factor2_variable_values = {
                key: value for key, value in values.items() if key in other.scope
            }
            return self.pdf(factor1_variable_values) * other.pdf(
                factor2_variable_values
            )
...
class BayesianNetwork:
    ...
    def joint_pdf(self, variable_values) -> float:
        ...
        if not set(self.nodes.keys()).issubset(set(variable_values.keys())):
            raise TypeError(
                "Not all variables in the network are present in the values dictionary."
            )
    ...
    def infer(
        ...
        elimination_variables = (
            set(self.nodes.keys()) - {query_variable} - set(evidence.keys())
        )
