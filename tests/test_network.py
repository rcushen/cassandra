...
def test_root_node_creation():
    node = Node(
        "A", (0, 1), [], marginal_pdf=norm.pdf, parameters={"loc": 0, "scale": 1}
    )
...
def test_error_root_node_creation():
    with pytest.raises(
        ValueError,
        match="Root nodes must have a marginal probability distribution and parameters.",
    ):
        Node("A", (0, 1), [])
...
def test_conditional_pdf():
    ...
    assert_approx_equal(node.conditional_pdf(2, {"A": 1}), norm.pdf(2, loc=2, scale=1))
...
def test_factor_creation_child_node():
    ...
    assert_approx_equal(factor.pdf({"B": 1.5, "A": 2}), norm.pdf(1.5, loc=4, scale=1))
...
def test_error_bayesian_network_creation_invalid_node():
    ...
    nodeB = Node("B", (0, 2), ["C"], equation=lambda A: 2 * A)
...
def test_bayesian_network_joint_pdf():
    ...
    nodeB = Node("B", (0, 2), ["A"], equation=lambda A: 2 * A)
    nodeC = Node(
        "C",
        (0, 6),
        ["B"],
...
def test_marginalise_factor():
    parent_node = Node("parent", (0, 1), [], marginal_pdf=lambda x: 1.0, parameters={})
    child_node = Node(
        "child",
        domain=(-20, 21),
    ...
    values = {"child": 0.5}
    expected_marginal_prob = norm.cdf(1, loc=0.5, scale=1) - norm.cdf(
        0, loc=0.5, scale=1
    ...
    assert_approx_equal(expected_marginal_prob, computed_marginal_prob, 0.001)
...
def test_inference_simple_network():
    ...
    expected_probability = nodeA.marginal_pdf(query_range[0]) * nodeC.conditional_pdf(
        evidence["C"], {"B": equation_func(query_range[0])}
    ) + nodeA.marginal_pdf(query_range[1]) * nodeC.conditional_pdf(
        evidence["C"], {"B": equation_func(query_range[1])}
    )
...
def test_inference_complex_network():
    ...
    nodeB = Node(
        "B", (0, 5), [], marginal_pdf=norm.pdf, parameters={"loc": 3, "scale": 1}
    )
    nodeC = Node(
        "C",
        (-10, 10),
    ...
    inferred_probability = network.infer(query_variable, query_range, evidence)
    expected_probability = network.joint_pdf(
        {**evidence, query_variable: query_range[0]}
    ) + network.joint_pdf({**evidence, query_variable: query_range[1]})
